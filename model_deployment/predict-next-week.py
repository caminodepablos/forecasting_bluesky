...
st.title(':gray[PREDICT NEXT WEEK USERS & ACTIVITY]')
st.subheader(':gray[Predicción del número de nuevos usuarios y nivel de actividad que habrá la semana que viene en Bluesky]')
...
def generate_weekly_df(df, period='W'):
    '''
    Receives a daily dataframe with a "date" column (not datetime index),
    and returns the aggregated dataframe by period:
    ...
    # Weekly total users
    users = df[['users']].resample(period).last()

    # Aggregated news (categorical/text fields)
    num_ft = df._get_numeric_data().columns
    cat_ft = df.drop(columns=num_ft).columns
    news = df[cat_ft].resample(period).agg(lambda x: ' '.join(x.dropna().astype(str)))

    # Concat
    new_df = pd.concat([news, activity, users], axis=1)

    # New_users
    new_df['new_users'] = new_df['users'].diff().fillna(0)
...
col3.metric("Impact Score", f"{round(num_data['impact_score'].iloc[-2],2)}", f"{round(num_data['impact_score'].iloc[-2] - num_data['impact_score'].iloc[-3], 2)}")
...
if st.button("Predict Next Week New Users & Activity 🎱", type='primary', use_container_width=True):
    try:
